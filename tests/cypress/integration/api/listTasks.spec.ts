...
describe('List Tasks via API - mutation.admin.jahia.listTasks', () => {
    ...
    it('Get List of tasks', () => {
        ...
        cy.apolloQuery(apollo(), { query: GQL_LIST_TASKS }).its('data.admin.jahia.tasks.length').should('equals', 3)
    ...
    it('Get List of tasks empty list', () => {
        cy.apolloQuery(apollo(), { query: GQL_LIST_TASKS }).its('data.admin.jahia.tasks.length').should('equals', 0)
    })
})
